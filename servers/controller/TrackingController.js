...
const RegisterCourier = async (req, res) => {
  try {
    ...
    // Ensure packageTypes is an array
    if (typeof packageTypes === 'string') {
      packageTypes = packageTypes.split(',').map(item => item.trim());
    } else if (!Array.isArray(packageTypes)) {
      packageTypes = [];
    }

    // Convert packageTypes array back to a string for storage
    packageTypes = packageTypes.join(',');

    // Handle weight conversion (assuming it's "8kg")
    let numericWeight = parseFloat(weight.replace('kg', '').trim());
    if (isNaN(numericWeight)) {
      numericWeight = 0;  // Default to 0 if invalid
    }
...
const getAllShipments = async (req, res) => {
  ...
  const updateShipment = async (req, res) => {
    const { trackingCode } = req.params;
    const { username, email, contact, packageName, packageTypes, weight, dimensions, shipmentStatus } = req.body;
  ...
  module.exports={deleteShipment,updateShipment, getAllShipments, getShipmentByTrackingCode, RegisterCourier,deleteAllShipments}